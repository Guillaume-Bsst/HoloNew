...
# Copied from holosoma_retargeting's setup_retargeting.sh and adapted for HoloNew:
#  - dedicated env name 'holonew' and workspace ~/.holonew_deps (isolated from holosoma)
#  - the HoloNew package lives at the repo root (setup.py/pyproject.toml there),
#    so we `pip install -e $ROOT_DIR` instead of a nested src/ package.
...
# Use CONDA_ENV_NAME if provided, otherwise default to "holonew"
...
if [[ ! -f $SENTINEL_FILE ]]; then
  ...
  # Install HoloNew (package at the repo root)
  pip install -U pip
