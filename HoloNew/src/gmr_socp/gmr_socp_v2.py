...
class GmrSocpRetargeterV2:
    """Position + orientation tracking GMR-SOCP retargeter (v2).
    ...
    def __init__(
        ...
        **_ignored,
        ...
        print(f"[GmrSocpV2] Loading robot model from: {robot_xml_path}")
        ...
        for frame in IK_MATCH_TABLE1:
            ...
            if bid == -1:
                raise ValueError(
                    f"[GmrSocpV2] Body '{actual}' (mapped from table key '{frame}') "
                    f"not found in model. Available: {sorted(available_bodies)}"
                )
            if actual != frame:
                print(f"[GmrSocpV2] Remapped body: '{frame}' -> '{actual}'")
