...
def _fake_rt(T):
    ...
    return types.SimpleNamespace(
        gmr_stages=None, gmr_floor=None,
...
def test_window_slices_stage_object_pose():
    # The grounded object lives in gmr_stages["floor"]["object_pose"] and must be
    ...
    rt = types.SimpleNamespace(
        gmr_stages={"floor": ground}, gmr_floor=ground,
        gmr_grounded=np.zeros((T, 52, 3)),
    ...
    _window_solve_frames(rt, start)
    assert rt.gmr_stages["floor"]["object_pose"].shape[0] == T - start
    # local frame 0 maps to global frame `start`
    np.testing.assert_array_equal(
        rt.gmr_stages["floor"]["object_pose"][0], np.arange(start * 7, start * 7 + 7))
