...
prepare public surface: ``(GroundedScene, InteractionContext)`` — the grounding ``Calibration`` rides
inside ``grounded.calibration``, the subject body inside ``grounded.body``, the (human, robot) scale
inside the context. ``process_frame`` and ``trace_frame`` share ONE dataflow core (``_build_frame``)
...
from ..prepare.contracts import GroundedScene, InteractionContext
...
def frame_pose(grounded: GroundedScene, f: int) -> FramePose:
    """Per-frame world transforms, computed ONCE: SMPL bone (R, t) from the body's FK and each
    object's (R, t) from its grounded pose7. ``interaction`` poses its clouds from these (and reuses
    the object (R, t) as each object channel's frame in the eval); ``style`` needs none of it — it
    reads the demo joints straight from ``grounded``. ``body is None`` (positions-only) => no bones."""
...
def _build_frame(grounded: GroundedScene, ctx: InteractionContext, f: int, prof=NULL):
    ...
    instrumentation (spans) lives here, in the orchestrator — the pure ops stay clean."""
    with prof.span("frame", f=f):
        with prof.span("pose"):
            pose = frame_pose(grounded, f)
        with prof.span("style"):
            style_t = style.build(grounded, f)
...
def process_frame(grounded: GroundedScene, ctx: InteractionContext, f: int, prof=NULL) -> FrameTargets:
    """One frame -> ``FrameTargets`` (lean, prod path)."""
    *_, targets = _build_frame(grounded, ctx, f, prof)
    return targets


def trace_frame(grounded: GroundedScene, ctx: InteractionContext, f: int, prof=NULL) -> FrameTrace:
    """Same pure ops as ``process_frame``, intermediates kept -> ``FrameTrace`` (the seam for ``viz``)."""
    pose, human_world, object_worlds, human_field, targets = _build_frame(grounded, ctx, f, prof)
...
def run_sequence(grounded: GroundedScene, ctx: InteractionContext, prof=NULL) -> list[FrameTargets]:
    ...
    with prof.span("sequence", T=grounded.n_frames):
        return [process_frame(grounded, ctx, f, prof) for f in range(grounded.n_frames)]
